...
COPY_ES: dict[str, tuple[str, str]] = {
    ...
    'ALG-COM-001': (
        'Vas a ver que un número complejo \(z=a+bi\) es un punto del plano: \(a\) es horizontal y \(b\) vertical.',
        'Arrastra el punto o cambia \(a\) y \(b\): observa cómo Re\((z)\) e Im\((z)\) fijan la posición.',
    ),
    'ALG-COM-002': (
        'Vas a ver que el conjugado conserva la parte real y cambia el signo de la imaginaria: reflejo respecto del eje real.',
        'Arrastra \(z\) y observa cómo \(\bar z\) se mueve simétricamente al otro lado del eje real.',
    ),
    'ALG-COM-003': (
        'Vas a ver que el módulo mide la distancia de \(z\) al origen: \(|z|=\sqrt{a^2+b^2}\).',
        'Arrastra \(z\) y observa la longitud del vector; activa las componentes para ver el triángulo.',
    ),
    'ALG-COM-004': (
        'Vas a ver que en forma polar \(z\) se describe por distancia \(r\) y ángulo \(\theta\), equivalentes a \(a+bi\) y \(re^{i\theta}\).',
        'Mueve \(r\) y \(\theta\): al fijar \(r\) y girar \(\theta\), el punto recorre una circunferencia.',
    ),
    'ALG-COM-005': (
        'Vas a ver cómo \(\theta\) determina un punto del círculo unitario mediante \(\cos\theta\) y \(\sin\theta\): ese punto es \(e^{i\theta}\).',
        'Mueve \(\theta\) y observa ángulo, coordenadas y el número complejo a la vez.',
    ),
    'ALG-COM-006': (
        'Vas a ver que elevar un complejo a \(n\) multiplica el ángulo por \(n\) y eleva el módulo a la potencia \(n\).',
        'Cambia \(r\), \(\theta\) y \(n\): compara \(z\) con \(z^n\) y mira longitud y ángulo.',
    ),
    'ALG-COM-007': (
        'Vas a ver que las \(n\) raíces tienen módulo \(r^{1/n}\) y se distribuyen uniformemente alrededor del origen.',
...
COPY_I18N: dict[str, dict[str, tuple[str, str]]] = {
    'en': {
        ...
        'ALG-COM-001': (
        "You'll see that a complex number \(z=a+bi\) is a point in the plane: \(a\) is horizontal and \(b\) vertical.",
        'Drag the point or change \(a\) and \(b\): watch how Re\((z)\) and Im\((z)\) fix the position.',
    ),
        'ALG-COM-002': (
        "You'll see that the conjugate keeps the real part and flips the imaginary sign: a reflection across the real axis.",
        'Drag \(z\) and watch \(\bar z\) move symmetrically to the other side of the real axis.',
    ),
        'ALG-COM-003': (
        "You'll see that the modulus measures the distance from \(z\) to the origin: \(|z|=\sqrt{a^2+b^2}\).",
        'Drag \(z\) and watch the vector length; enable components to see the triangle.',
    ),
        'ALG-COM-004': (
        "You'll see that in polar form \(z\) is described by distance \(r\) and angle \(\theta\), matching \(a+bi\) and \(re^{i\theta}\).",
        'Move \(r\) and \(\theta\): with fixed \(r\), turning \(\theta\) traces a circle.',
    ),
        'ALG-COM-005': (
        "You'll see how \(\theta\) picks a point on the unit circle via \(\cos\theta\) and \(\sin\theta\): that point is \(e^{i\theta}\).",
        'Move \(\theta\) and watch angle, coordinates, and the complex number together.',
    ),
        'ALG-COM-006': (
        "You'll see that raising a complex number to \(n\) multiplies the angle by \(n\) and raises the modulus to the \(n\)th power.",
        'Change \(r\), \(\theta\) and \(n\): compare \(z\) with \(z^n\) and watch length and angle.',
    ),
        'ALG-COM-007': (
        "You'll see that the \(n\) roots share modulus \(r^{1/n}\) and sit evenly around the origin.",
    ...
    'de': {
        ...
        'ALG-COM-001': (
        'Du siehst: Eine komplexe Zahl \(z=a+bi\) ist ein Punkt in der Ebene: \(a\) horizontal, \(b\) vertikal.',
        'Ziehe den Punkt oder ändere \(a\) und \(b\): Re\((z)\) und Im\((z)\) bestimmen die Lage.',
    ),
        'ALG-COM-002': (
        'Du siehst: Das Konjugierte behält den Realteil und kehrt das Vorzeichen des Imaginärteils um — Spiegelung an der reellen Achse.',
        'Ziehe \(z\) und sieh, wie \(\bar z\) symmetrisch zur anderen Seite wandert.',
    ),
        'ALG-COM-003': (
        'Du siehst: Der Betrag misst den Abstand von \(z\) zum Ursprung: \(|z|=\sqrt{a^2+b^2}\).',
        'Ziehe \(z\) und sieh die Vektorlänge; aktiviere Komponenten für das Dreieck.',
    ),
        'ALG-COM-004': (
        'Du siehst: In Polarform beschreiben Abstand \(r\) und Winkel \(\theta\) die Zahl, gleichwertig zu \(a+bi\) und \(re^{i\theta}\).',
        'Bewege \(r\) und \(\theta\): bei festem \(r\) läuft der Punkt auf einem Kreis.',
    ),
        'ALG-COM-005': (
        'Du siehst: \(\theta\) wählt einen Punkt auf dem Einheitskreis über \(\cos\theta\) und \(\sin\theta\) — das ist \(e^{i\theta}\).',
        'Bewege \(\theta\) und sieh Winkel, Koordinaten und die komplexe Zahl zugleich.',
    ),
        'ALG-COM-006': (
        'Du siehst: Erheben auf \(n\) multipliziert den Winkel mit \(n\) und potenziert den Betrag mit \(n\).',
        'Ändere \(r\), \(\theta\) und \(n\): vergleiche \(z\) mit \(z^n\).',
    ),
        'ALG-COM-007': (
        'Du siehst: Die \(n\) Wurzeln haben Betrag \(r^{1/n}\) und liegen gleichmäßig um den Ursprung.',
        'Ändere \(n\), \(r\) und \(\theta\): die Wurzeln bilden ein regelmäßiges Polygon mit Abstand \(2\pi/n\).',
    ),
        'ALG-SEC-001': (
    ...
    'fr': {
        ...
        'ALG-COM-001': (
        'Tu vas voir qu’un complexe \(z=a+bi\) est un point du plan : \(a\) horizontal, \(b\) vertical.',
        'Traîne le point ou change \(a\) et \(b\) : Re\((z)\) et Im\((z)\) fixent la position.',
    ),
        'ALG-COM-002': (
        'Tu vas voir que le conjugué garde la partie réelle et change le signe de l’imaginaire : réflexion sur l’axe réel.',
        'Traîne \(z\) et observe \(\bar z\) se déplacer symétriquement de l’autre côté.',
    ),
        'ALG-COM-003': (
        'Tu vas voir que le module mesure la distance de \(z\) à l’origine : \(|z|=\sqrt{a^2+b^2}\).',
        'Traîne \(z\) et observe la longueur du vecteur ; active les composantes pour le triangle.',
    ),
        'ALG-COM-004': (
        'Tu vas voir qu’en polaire, \(z\) se décrit par \(r\) et \(\theta\), équivalents à \(a+bi\) et \(re^{i\theta}\).',
        'Bouge \(r\) et \(\theta\) : avec \(r\) fixe, tourner \(\theta\) parcourt un cercle.',
    ),
        'ALG-COM-005': (
        'Tu vas voir comment \(\theta\) choisit un point du cercle unité via \(\cos\theta\) et \(\sin\theta\) : c’est \(e^{i\theta}\).',
        'Bouge \(\theta\) et observe angle, coordonnées et nombre complexe ensemble.',
    ),
        'ALG-COM-006': (
        'Tu vas voir qu’élever à \(n\) multiplie l’angle par \(n\) et élève le module à la puissance \(n\).',
        'Change \(r\), \(\theta\) et \(n\) : compare \(z\) et \(z^n\).',
    ),
        'ALG-COM-007': (
        'Tu vas voir que les \(n\) racines ont module \(r^{1/n}\) et se répartissent uniformément autour de l’origine.',
        'Change \(n\), \(r\) et \(\theta\) : les racines forment un polygone régulier espacé de \(2\pi/n\).',
    ),
        'ALG-SEC-001': (
    ...
    'it': {
        ...
        'ALG-COM-001': (
        'Vedrai che un complesso \(z=a+bi\) è un punto del piano: \(a\) orizzontale e \(b\) verticale.',
        'Trascina il punto o cambia \(a\) e \(b\): Re\((z)\) e Im\((z)\) fissano la posizione.',
    ),
        'ALG-COM-002': (
        'Vedrai che il coniugato conserva la parte reale e cambia il segno dell’immaginaria: riflessione sull’asse reale.',
        'Trascina \(z\) e osserva \(\bar z\) muoversi simmetricamente dall’altra parte.',
    ),
        'ALG-COM-003': (
        'Vedrai che il modulo misura la distanza di \(z\) dall’origine: \(|z|=\sqrt{a^2+b^2}\).',
        'Trascina \(z\) e osserva la lunghezza del vettore; attiva le componenti per il triangolo.',
    ),
        'ALG-COM-004': (
        'Vedrai che in forma polare \(z\) si descrive con \(r\) e \(\theta\), equivalenti a \(a+bi\) e \(re^{i\theta}\).',
        'Muovi \(r\) e \(\theta\): con \(r\) fisso, girare \(\theta\) percorre una circonferenza.',
    ),
        'ALG-COM-005': (
        'Vedrai come \(\theta\) sceglie un punto del cerchio unitario con \(\cos\theta\) e \(\sin\theta\): quel punto è \(e^{i\theta}\).',
        'Muovi \(\theta\) e osserva angolo, coordinate e numero complesso insieme.',
    ),
        'ALG-COM-006': (
        'Vedrai che elevare a \(n\) moltiplica l’angolo per \(n\) e eleva il modulo alla potenza \(n\).',
        'Cambia \(r\), \(\theta\) e \(n\): confronta \(z\) con \(z^n\).',
    ),
        'ALG-COM-007': (
        'Vedrai che le \(n\) radici hanno modulo \(r^{1/n}\) e si distribuiscono uniformemente intorno all’origine.',
        'Cambia \(n\), \(r\) e \(\theta\): le radici formano un poligono regolare separato da \(2\pi/n\).',
    ),
        'ALG-SEC-001': (
    ...
    'pt': {
        ...
        'ALG-COM-001': (
        'Vais ver que um complexo \(z=a+bi\) é um ponto do plano: \(a\) é horizontal e \(b\) vertical.',
        'Arrasta o ponto ou muda \(a\) e \(b\): Re\((z)\) e Im\((z)\) fixam a posição.',
    ),
        'ALG-COM-002': (
        'Vais ver que o conjugado conserva a parte real e muda o sinal da imaginária: reflexão no eixo real.',
        'Arrasta \(z\) e observa \(\bar z\) mover-se simetricamente para o outro lado.',
    ),
        'ALG-COM-003': (
        'Vais ver que o módulo mede a distância de \(z\) à origem: \(|z|=\sqrt{a^2+b^2}\).',
        'Arrasta \(z\) e observa o comprimento do vetor; ativa as componentes para o triângulo.',
    ),
        'ALG-COM-004': (
        'Vais ver que na forma polar \(z\) descreve-se com \(r\) e \(\theta\), equivalentes a \(a+bi\) e \(re^{i\theta}\).',
        'Move \(r\) e \(\theta\): com \(r\) fixo, girar \(\theta\) percorre uma circunferência.',
    ),
        'ALG-COM-005': (
        'Vais ver como \(\theta\) escolhe um ponto do círculo unitário via \(\cos\theta\) e \(\sin\theta\): esse ponto é \(e^{i\theta}\).',
        'Move \(\theta\) e observa ângulo, coordenadas e o número complexo ao mesmo tempo.',
    ),
        'ALG-COM-006': (
        'Vais ver que elevar a \(n\) multiplica o ângulo por \(n\) e eleva o módulo à potência \(n\).',
        'Muda \(r\), \(\theta\) e \(n\): compara \(z\) com \(z^n\).',
    ),
        'ALG-COM-007': (
        'Vais ver que as \(n\) raízes têm módulo \(r^{1/n}\) e distribuem-se uniformemente em torno da origem.',
        'Muda \(n\), \(r\) e \(\theta\): as raízes formam um polígono regular separado por \(2\pi/n\).',
    ),
        'ALG-SEC-001': (
