...
TITLE_ROWS: list[tuple[str, str, str, str, str, str]] = [
    ...
    ("Máxima transferencia de potencia", "Maximum power transfer", "Maximale Leistungsübertragung", "Transfert maximal de puissance", "Massimo trasferimento di potenza", "Máxima transferência de potência"),
    ...
    ("Umbral de conmutación", "Switching threshold", "Schaltschwelle", "Seuil de commutation", "Soglia di commutazione", "Limiar de comutação"),
...
VARS_I18N = {
    ...
    "de": {
        "DIV": r"\(V_i,V_o\): Spannungen (V); \(R_1,R_2,R_L,R_{\mathrm{Th}}\): Widerstände (Ω); \(I_T,I_N\): Ströme (A).",
        "REA": r"\(C,C_{\mathrm{eq}}\): Kapazitäten (F); \(L,M\): Induktivitäten (H); \(v,i\): Spannung (V) und Strom (A); \(\tau\): Zeitkonstante (s); \(k\): Kopplung (dimensionslos).",
        "TRN": r"\(v(t),v_0,v_\infty\): Spannungen (V); \(i(t),i_0,i_\infty\): Ströme (A); \(\tau\): Zeitkonstante (s); \(t\): Zeit (s); \(R,C,L\): Ω, F, H.",
        "RLC": r"\(\omega_0,\omega_d\): Frequenzen (rad/s); \(\alpha\): Dämpfung (s^{-1}); \(\zeta,Q\): dimensionslos; \(R,L,C\): Ω, H, F.",
        "FAS": r"\(V_p\): Spitze (V); \(V_{\mathrm{rms}}\): Effektivwert (V); \(\omega\): rad/s; \(f\): Hz; \(\phi\): rad; \(Z,X\): Ω; \(Y\): S.",
        "PAC": r"\(P\): W; \(Q\): var; \(S\): VA; \(V,I\): RMS (V, A); \(\theta\): rad; \(\omega_0\): rad/s; \(R,L,C\): Ω, H, F.",
        "FIL": r"\(H(j\omega)\): dimensionslos; \(\omega\): rad/s; \(f_c,f_0\): Hz; \(R,C,L\): Ω, F, H; \(Q\): dimensionslos.",
        "XFR": r"\(V_1,V_2\): V; \(I_1,I_2\): A; \(N_1,N_2,n\): Windungen oder Verhältnis; \(Z_L,Z_{\mathrm{in}}\): Ω; \(\eta\): dimensionslos.",
        "DIO": r"\(v,V_F,V_Z,V_T\): V; \(I,I_s\): A; \(n\): Idealitätsfaktor; \(E_g\): eV; \(f,R,C\): Hz, Ω, F.",
        "BJT": r"\(I_C,I_B,I_E\): A; \(\alpha,\beta\): dimensionslos; \(V_{CE},V_{BE}\): V; \(g_m\): S; \(r_\pi\): Ω.",
        "FET": r"\(I_D,I_{DSS}\): A; \(V_{GS},V_{th},V_{DS}\): V; \(k_n\): A/V²; \(R_{DS(\mathrm{on})}\): Ω; \(g_m\): S.",
        "OPA": r"\(v_+,v_-,v_o,v_{\mathrm{in}}\): V; \(A\): Verstärkung (dimensionslos); \(R_f,R_{\mathrm{in}},R_g\): Ω; \(\mathrm{CMRR}\): dimensionslos.",
        "LGC": r"\(V_{OH},V_{OL},V_{IH},V_{IL},V_M\): V; \(t_{pHL},t_p\): s; \(R_{\mathrm{pu}}\): Ω; Fan-out und Fan-in: dimensionslos.",
        "CMB": r"Logikeingänge und -ausgänge \(A,B,S,Y,C_{\mathrm{out}}\): \{0,1\}. Die Karnaugh-Tafel minimiert; sie katalogisiert Identitäten (`ALG-BOO-*`) nicht neu.",
        "SEQ": r"\(D,J,K,T,Q\): Bits; \(t_{su},t_h,t_{pcq}\): s; \(n\): Bits oder Modul; \(f_{\max}\): Hz.",
        "ADC": r"\(f_s,f_{\max},f_N\): Hz; \(n\): Bits; \(V_{\mathrm{FS}}\): V; \(\Delta\): V; \(\delta\): dimensionslos; \(t_{\mathrm{on}},T\): s.",
    },
    "fr": {
        "DIV": r"\(V_i,V_o\) : tensions (V) ; \(R_1,R_2,R_L,R_{\mathrm{Th}}\) : résistances (Ω) ; \(I_T,I_N\) : courants (A).",
        "REA": r"\(C,C_{\mathrm{eq}}\) : capacités (F) ; \(L,M\) : inductances (H) ; \(v,i\) : tension (V) et courant (A) ; \(\tau\) : constante de temps (s) ; \(k\) : couplage (adimensionnel).",
        "TRN": r"\(v(t),v_0,v_\infty\) : tensions (V) ; \(i(t),i_0,i_\infty\) : courants (A) ; \(\tau\) : constante de temps (s) ; \(t\) : temps (s) ; \(R,C,L\) : Ω, F, H.",
        "RLC": r"\(\omega_0,\omega_d\) : fréquences (rad/s) ; \(\alpha\) : amortissement (s^{-1}) ; \(\zeta,Q\) : adimensionnels ; \(R,L,C\) : Ω, H, F.",
        "FAS": r"\(V_p\) : crête (V) ; \(V_{\mathrm{rms}}\) : efficace (V) ; \(\omega\) : rad/s ; \(f\) : Hz ; \(\phi\) : rad ; \(Z,X\) : Ω ; \(Y\) : S.",
        "PAC": r"\(P\) : W ; \(Q\) : var ; \(S\) : VA ; \(V,I\) : RMS (V, A) ; \(\theta\) : rad ; \(\omega_0\) : rad/s ; \(R,L,C\) : Ω, H, F.",
        "FIL": r"\(H(j\omega)\) : adimensionnel ; \(\omega\) : rad/s ; \(f_c,f_0\) : Hz ; \(R,C,L\) : Ω, F, H ; \(Q\) : adimensionnel.",
        "XFR": r"\(V_1,V_2\) : V ; \(I_1,I_2\) : A ; \(N_1,N_2,n\) : spires ou rapport ; \(Z_L,Z_{\mathrm{in}}\) : Ω ; \(\eta\) : adimensionnel.",
        "DIO": r"\(v,V_F,V_Z,V_T\) : V ; \(I,I_s\) : A ; \(n\) : facteur d'idéalité ; \(E_g\) : eV ; \(f,R,C\) : Hz, Ω, F.",
        "BJT": r"\(I_C,I_B,I_E\) : A ; \(\alpha,\beta\) : adimensionnels ; \(V_{CE},V_{BE}\) : V ; \(g_m\) : S ; \(r_\pi\) : Ω.",
        "FET": r"\(I_D,I_{DSS}\) : A ; \(V_{GS},V_{th},V_{DS}\) : V ; \(k_n\) : A/V² ; \(R_{DS(\mathrm{on})}\) : Ω ; \(g_m\) : S.",
        "OPA": r"\(v_+,v_-,v_o,v_{\mathrm{in}}\) : V ; \(A\) : gain (adimensionnel) ; \(R_f,R_{\mathrm{in}},R_g\) : Ω ; \(\mathrm{CMRR}\) : adimensionnel.",
        "LGC": r"\(V_{OH},V_{OL},V_{IH},V_{IL},V_M\) : V ; \(t_{pHL},t_p\) : s ; \(R_{\mathrm{pu}}\) : Ω ; fan-out et fan-in : adimensionnels.",
        "CMB": r"Entrées et sorties logiques \(A,B,S,Y,C_{\mathrm{out}}\) : \{0,1\}. La carte de Karnaugh minimise ; elle ne recatalogue pas les identités (`ALG-BOO-*`).",
        "SEQ": r"\(D,J,K,T,Q\) : bits ; \(t_{su},t_h,t_{pcq}\) : s ; \(n\) : bits ou module ; \(f_{\max}\) : Hz.",
        "ADC": r"\(f_s,f_{\max},f_N\) : Hz ; \(n\) : bits ; \(V_{\mathrm{FS}}\) : V ; \(\Delta\) : V ; \(\delta\) : adimensionnel ; \(t_{\mathrm{on}},T\) : s.",
    },
    "it": {
        "DIV": r"\(V_i,V_o\): tensioni (V); \(R_1,R_2,R_L,R_{\mathrm{Th}}\): resistenze (Ω); \(I_T,I_N\): correnti (A).",
        "REA": r"\(C,C_{\mathrm{eq}}\): capacità (F); \(L,M\): induttanze (H); \(v,i\): tensione (V) e corrente (A); \(\tau\): costante di tempo (s); \(k\): accoppiamento (adimensionale).",
        "TRN": r"\(v(t),v_0,v_\infty\): tensioni (V); \(i(t),i_0,i_\infty\): correnti (A); \(\tau\): costante di tempo (s); \(t\): tempo (s); \(R,C,L\): Ω, F, H.",
        "RLC": r"\(\omega_0,\omega_d\): frequenze (rad/s); \(\alpha\): smorzamento (s^{-1}); \(\zeta,Q\): adimensionali; \(R,L,C\): Ω, H, F.",
        "FAS": r"\(V_p\): picco (V); \(V_{\mathrm{rms}}\): efficace (V); \(\omega\): rad/s; \(f\): Hz; \(\phi\): rad; \(Z,X\): Ω; \(Y\): S.",
        "PAC": r"\(P\): W; \(Q\): var; \(S\): VA; \(V,I\): RMS (V, A); \(\theta\): rad; \(\omega_0\): rad/s; \(R,L,C\): Ω, H, F.",
        "FIL": r"\(H(j\omega)\): adimensionale; \(\omega\): rad/s; \(f_c,f_0\): Hz; \(R,C,L\): Ω, F, H; \(Q\): adimensionale.",
        "XFR": r"\(V_1,V_2\): V; \(I_1,I_2\): A; \(N_1,N_2,n\): spire o rapporto; \(Z_L,Z_{\mathrm{in}}\): Ω; \(\eta\): adimensionale.",
        "DIO": r"\(v,V_F,V_Z,V_T\): V; \(I,I_s\): A; \(n\): fattore di idealità; \(E_g\): eV; \(f,R,C\): Hz, Ω, F.",
        "BJT": r"\(I_C,I_B,I_E\): A; \(\alpha,\beta\): adimensionali; \(V_{CE},V_{BE}\): V; \(g_m\): S; \(r_\pi\): Ω.",
        "FET": r"\(I_D,I_{DSS}\): A; \(V_{GS},V_{th},V_{DS}\): V; \(k_n\): A/V²; \(R_{DS(\mathrm{on})}\): Ω; \(g_m\): S.",
        "OPA": r"\(v_+,v_-,v_o,v_{\mathrm{in}}\): V; \(A\): guadagno (adimensionale); \(R_f,R_{\mathrm{in}},R_g\): Ω; \(\mathrm{CMRR}\): adimensionale.",
        "LGC": r"\(V_{OH},V_{OL},V_{IH},V_{IL},V_M\): V; \(t_{pHL},t_p\): s; \(R_{\mathrm{pu}}\): Ω; fan-out e fan-in: adimensionali.",
        "CMB": r"Ingressi e uscite logiche \(A,B,S,Y,C_{\mathrm{out}}\): \{0,1\}. La mappa di Karnaugh minimizza; non ricataloga le identità (`ALG-BOO-*`).",
        "SEQ": r"\(D,J,K,T,Q\): bit; \(t_{su},t_h,t_{pcq}\): s; \(n\): bit o modulo; \(f_{\max}\): Hz.",
        "ADC": r"\(f_s,f_{\max},f_N\): Hz; \(n\): bit; \(V_{\mathrm{FS}}\): V; \(\Delta\): V; \(\delta\): adimensionale; \(t_{\mathrm{on}},T\): s.",
    },
    "pt": {
        "DIV": r"\(V_i,V_o\): tensões (V); \(R_1,R_2,R_L,R_{\mathrm{Th}}\): resistências (Ω); \(I_T,I_N\): correntes (A).",
        "REA": r"\(C,C_{\mathrm{eq}}\): capacitâncias (F); \(L,M\): indutâncias (H); \(v,i\): tensão (V) e corrente (A); \(\tau\): constante de tempo (s); \(k\): acoplamento (adimensional).",
        "TRN": r"\(v(t),v_0,v_\infty\): tensões (V); \(i(t),i_0,i_\infty\): correntes (A); \(\tau\): constante de tempo (s); \(t\): tempo (s); \(R,C,L\): Ω, F, H.",
        "RLC": r"\(\omega_0,\omega_d\): frequências (rad/s); \(\alpha\): amortecimento (s^{-1}); \(\zeta,Q\): adimensionais; \(R,L,C\): Ω, H, F.",
        "FAS": r"\(V_p\): pico (V); \(V_{\mathrm{rms}}\): eficaz (V); \(\omega\): rad/s; \(f\): Hz; \(\phi\): rad; \(Z,X\): Ω; \(Y\): S.",
        "PAC": r"\(P\): W; \(Q\): var; \(S\): VA; \(V,I\): RMS (V, A); \(\theta\): rad; \(\omega_0\): rad/s; \(R,L,C\): Ω, H, F.",
        "FIL": r"\(H(j\omega)\): adimensional; \(\omega\): rad/s; \(f_c,f_0\): Hz; \(R,C,L\): Ω, F, H; \(Q\): adimensional.",
        "XFR": r"\(V_1,V_2\): V; \(I_1,I_2\): A; \(N_1,N_2,n\): espiras ou relação; \(Z_L,Z_{\mathrm{in}}\): Ω; \(\eta\): adimensional.",
        "DIO": r"\(v,V_F,V_Z,V_T\): V; \(I,I_s\): A; \(n\): fator de idealidade; \(E_g\): eV; \(f,R,C\): Hz, Ω, F.",
        "BJT": r"\(I_C,I_B,I_E\): A; \(\alpha,\beta\): adimensionais; \(V_{CE},V_{BE}\): V; \(g_m\): S; \(r_\pi\): Ω.",
        "FET": r"\(I_D,I_{DSS}\): A; \(V_{GS},V_{th},V_{DS}\): V; \(k_n\): A/V²; \(R_{DS(\mathrm{on})}\): Ω; \(g_m\): S.",
        "OPA": r"\(v_+,v_-,v_o,v_{\mathrm{in}}\): V; \(A\): ganho (adimensional); \(R_f,R_{\mathrm{in}},R_g\): Ω; \(\mathrm{CMRR}\): adimensional.",
        "LGC": r"\(V_{OH},V_{OL},V_{IH},V_{IL},V_M\): V; \(t_{pHL},t_p\): s; \(R_{\mathrm{pu}}\): Ω; fan-out e fan-in: adimensionais.",
        "CMB": r"Entradas e saídas lógicas \(A,B,S,Y,C_{\mathrm{out}}\): \{0,1\}. O mapa de Karnaugh minimiza; não recataloga identidades (`ALG-BOO-*`).",
        "SEQ": r"\(D,J,K,T,Q\): bits; \(t_{su},t_h,t_{pcq}\): s; \(n\): bits ou módulo; \(f_{\max}\): Hz.",
        "ADC": r"\(f_s,f_{\max},f_N\): Hz; \(n\): bits; \(V_{\mathrm{FS}}\): V; \(\Delta\): V; \(\delta\): adimensional; \(t_{\mathrm{on}},T\): s.",
    },
}
...
INTUITIVE_I18N["de"] = {
        "DIV": "Für „{title}“: Teiler und Thévenin/Norton reduzieren das Netz auf ein Tor; eine Last parallel zu R2 senkt Vo.",
        "REA": "Für „{title}“: C und L speichern Energie und halten vC und iL stetig; Reihe/Parallel ändern Ceq oder Leq.",
        "TRN": "Für „{title}“: die Exponentialfunktion interpoliert den stetigen Anfangswert und den Endwert mit τ.",
        "RLC": "Für „{title}“: ζ vergleicht α mit ω0 und entscheidet zwei Exponentiale, die Grenze oder eine gedämpfte Schwingung.",
        "FAS": "Für „{title}“: der RMS-Zeiger dreht mit ω; seine Projektion ist die Sinusgröße und Z = R + jX fasst das Element zusammen.",
        "PAC": "Für „{title}“: P, Q und S bilden das Leistungsdreieck; in Resonanz verschwindet die Netzreaktanz.",
        "FIL": "Für „{title}“: |H(jω)| zeigt das Durchlassband und fc markiert den −3-dB-Punkt im Bode-Diagramm.",
        "XFR": "Für „{title}“: n = N2/N1 skaliert Spannungen und kehrt Ströme um; ZL spiegelt sich als ZL/n².",
        "DIO": "Für „{title}“: die Diode leitet stark hinter der Schwelle; der Gleichrichter schneidet oder faltet die Sinusgröße.",
        "BJT": "Für „{title}“: IB setzt IC = β IB im aktiven Bereich; die Lastgerade läuft durch Sperr-, Aktiv- und Sättigungsbereich.",
        "FET": "Für „{title}“: VGS gegenüber Vth öffnet den Kanal; in Sättigung hängt ID von (VGS−Vth)² ab.",
        "OPA": "Für „{title}“: mit Gegenkopplung gilt v+ ≈ v− (virtueller Knoten) und das Widerstandsnetz setzt die Verstärkung.",
        "LGC": "Für „{title}“: VOH, VOL, VIH und VIL setzen die Rauschabstände und die CMOS-VTC invertiert zwischen den Rails.",
        "CMB": "Für „{title}“: Wahrheitstabelle und Karnaugh-Tafel minimieren Hardware; sie schreiben ALG-BOO-Identitäten nicht um.",
        "SEQ": "Für „{title}“: die Flanke tastet D ab; tsu und th markieren das Fenster, in dem eine Änderung Metastabilität erzeugt.",
        "ADC": "Für „{title}“: fs > 2 fmax vermeidet Aliasing; n Bits teilen VFS in 2^n Stufen und PWM nutzt den Tastgrad.",
    }
INTUITIVE_I18N["fr"] = {
        "DIV": "Pour « {title} » : le diviseur et Thévenin/Norton réduisent le réseau à un port ; une charge en parallèle avec R2 abaisse Vo.",
        "REA": "Pour « {title} » : C et L stockent l'énergie et gardent vC et iL continues ; série/parallèle changent Ceq ou Leq.",
        "TRN": "Pour « {title} » : l'exponentielle interpole la valeur initiale continue et la valeur finale avec τ.",
        "RLC": "Pour « {title} » : ζ compare α à ω0 et décide deux exponentielles, la frontière ou une oscillation amortie.",
        "FAS": "Pour « {title} » : le phaseur RMS tourne à ω ; sa projection est la sinusoïde et Z = R + jX résume l'élément.",
        "PAC": "Pour « {title} » : P, Q et S forment le triangle des puissances ; à la résonance la réactance nette s'annule.",
        "FIL": "Pour « {title} » : |H(jω)| montre la bande passante et fc marque le point −3 dB du Bode.",
        "XFR": "Pour « {title} » : n = N2/N1 met à l'échelle les tensions et inverse les courants ; ZL se ramène en ZL/n².",
        "DIO": "Pour « {title} » : la diode conduit fortement au-delà du seuil ; le redresseur écrête ou replie la sinusoïde.",
        "BJT": "Pour « {title} » : IB fixe IC = β IB en région active ; la droite de charge traverse blocage, active et saturation.",
        "FET": "Pour « {title} » : VGS par rapport à Vth ouvre le canal ; en saturation ID dépend de (VGS−Vth)².",
        "OPA": "Pour « {title} » : avec contre-réaction v+ ≈ v− (nœud virtuel) et le réseau de résistances fixe le gain.",
        "LGC": "Pour « {title} » : VOH, VOL, VIH et VIL fixent les marges de bruit et la VTC CMOS inverse entre les rails.",
        "CMB": "Pour « {title} » : table de vérité et carte de Karnaugh minimisent le matériel ; elles ne réécrivent pas les identités ALG-BOO.",
        "SEQ": "Pour « {title} » : le front échantillonne D ; tsu et th marquent la fenêtre où un changement provoque la métastabilité.",
        "ADC": "Pour « {title} » : fs > 2 fmax évite le repliement ; n bits partagent VFS en 2^n niveaux et le PWM utilise le rapport cyclique.",
    }
INTUITIVE_I18N["it"] = {
        "DIV": "Per «{title}»: partitore e Thévenin/Norton riducono la rete a una porta; un carico in parallelo a R2 abbassa Vo.",
        "REA": "Per «{title}»: C e L immagazzinano energia e tengono vC e iL continue; serie/parallelo cambiano Ceq o Leq.",
        "TRN": "Per «{title}»: l'esponenziale interpola il valore iniziale continuo e il valore finale con τ.",
        "RLC": "Per «{title}»: ζ confronta α con ω0 e decide due esponenziali, il confine o un'oscillazione smorzata.",
        "FAS": "Per «{title}»: il fasore RMS ruota a ω; la sua proiezione è la sinusoide e Z = R + jX riassume l'elemento.",
        "PAC": "Per «{title}»: P, Q e S formano il triangolo delle potenze; in risonanza la reattanza netta si annulla.",
        "FIL": "Per «{title}»: |H(jω)| mostra la banda passante e fc marca il punto −3 dB del Bode.",
        "XFR": "Per «{title}»: n = N2/N1 scala le tensioni e inverte le correnti; ZL si riflette come ZL/n².",
        "DIO": "Per «{title}»: il diodo conduce forte oltre la soglia; il raddrizzatore taglia o piega la sinusoide.",
        "BJT": "Per «{title}»: IB fissa IC = β IB in regione attiva; la retta di carico attraversa interdizione, attiva e saturazione.",
        "FET": "Per «{title}»: VGS rispetto a Vth apre il canale; in saturazione ID dipende da (VGS−Vth)².",
        "OPA": "Per «{title}»: con controreazione v+ ≈ v− (nodo virtuale) e la rete di resistenze fissa il guadagno.",
        "LGC": "Per «{title}»: VOH, VOL, VIH e VIL fissano i margini di rumore e la VTC CMOS inverte tra i rail.",
        "CMB": "Per «{title}»: tavola di verità e mappa di Karnaugh minimizzano l'hardware; non riscrivono le identità ALG-BOO.",
        "SEQ": "Per «{title}»: il fronte campiona D; tsu e th marcano la finestra in cui un cambiamento provoca metastabilità.",
        "ADC": "Per «{title}»: fs > 2 fmax evita l'aliasing; n bit dividono VFS in 2^n livelli e il PWM usa il duty.",
    }
INTUITIVE_I18N["pt"] = {
        "DIV": "Para «{title}»: o divisor e Thévenin/Norton reduzem a rede a uma porta; uma carga em paralelo com R2 baixa Vo.",
        "REA": "Para «{title}»: C e L armazenam energia e mantêm vC e iL contínuas; série/paralelo mudam Ceq ou Leq.",
        "TRN": "Para «{title}»: a exponencial interpola o valor inicial contínuo e o valor final com τ.",
        "RLC": "Para «{title}»: ζ compara α com ω0 e decide duas exponenciais, a fronteira ou uma oscilação amortecida.",
        "FAS": "Para «{title}»: o fasor RMS gira a ω; a sua projeção é a senoide e Z = R + jX resume o elemento.",
        "PAC": "Para «{title}»: P, Q e S formam o triângulo de potências; na ressonância a reatância líquida anula-se.",
        "FIL": "Para «{title}»: |H(jω)| mostra a banda passante e fc marca o ponto −3 dB do Bode.",
        "XFR": "Para «{title}»: n = N2/N1 escala tensões e inverte correntes; ZL reflete-se como ZL/n².",
        "DIO": "Para «{title}»: o diodo conduz forte após o limiar; o retificador recorta ou dobra a senoide.",
        "BJT": "Para «{title}»: IB fixa IC = β IB na região ativa; a reta de carga atravessa corte, ativa e saturação.",
        "FET": "Para «{title}»: VGS face a Vth abre o canal; em saturação ID depende de (VGS−Vth)².",
        "OPA": "Para «{title}»: com realimentação negativa v+ ≈ v− (nó virtual) e a rede de resistências fixa o ganho.",
        "LGC": "Para «{title}»: VOH, VOL, VIH e VIL definem as margens de ruído e a VTC CMOS inverte entre os carris.",
        "CMB": "Para «{title}»: tabela verdade e mapa de Karnaugh minimizam hardware; não reescrevem identidades ALG-BOO.",
        "SEQ": "Para «{title}»: a borda amostra D; tsu e th marcam a janela em que uma mudança provoca metastabilidade.",
        "ADC": "Para «{title}»: fs > 2 fmax evita aliasing; n bits dividem VFS em 2^n níveis e o PWM usa o duty.",
    }
