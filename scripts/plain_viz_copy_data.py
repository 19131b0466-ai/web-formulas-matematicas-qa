...
COPY_ES: dict[str, tuple[str, str]] = {
    ...
    "ALG-INE-002": (
        "La solución es un conjunto de valores de x (intervalos), no un “área del plano”.",
        "Elige la desigualdad y mueve a,b,c: usa la parábola para el signo y lee la solución en la recta.",
    ),
    "ALG-INE-003": (
        "Los ceros del denominador nunca pertenecen a la solución; la respuesta es una unión de intervalos.",
    ...
    "ALG-LOG-001": (
        "Cada avance de una unidad en x multiplica el valor por la base a: si a>1 crece y si 0<a<1 decrece.",
        "Mueve la base a y el valor x. Observa (0,1), la asíntota y=0 y cómo la curva crece o decrece.",
    ),
    "ALG-LOG-002": (
        "El logaritmo responde: “¿a qué exponente elevo la base para obtener x?”.",
        "Mueve la base y x. Si log_b(x)=y, entonces b^y=x. Los puntos (x,y) y (y,x) se reflejan respecto a y=x.",
    ),
    "ALG-LOG-007": (
        "El signo de k decide si la cantidad crece, decrece o permanece constante.",
        "Mueve k: positivo crece, negativo decrece y k=0 queda constante. Mueve t y observa P(t)=P₀ e^{kt}.",
    ),
    "ALG-COM-001": (
    ...
    "ALG-BOO-001": (
        "Vas a ver que 0 y 1 se comportan de forma opuesta en OR y AND: uno puede dejar A intacto o dominar completamente el resultado.",
        "Cambia A entre 0 y 1 y observa qué operaciones conservan su valor y cuáles lo fuerzan.",
    ),
    "ALG-BOO-002": (
        "Vas a ver dos patrones: repetir A conserva su valor; combinar A con su complemento produce siempre 1 con OR y 0 con AND.",
        "Cambia A entre 0 y 1 y observa qué resultados dependen de A y cuáles permanecen constantes.",
    ),
    "ALG-BOO-003": (
        "Vas a ver cómo un término se distribuye sobre cada rama de una expresión booleana.",
        "Distribuye A paso a paso y cambia A, B y C para comprobar que ambas formas siempre producen la misma salida.",
    ),
    "ALG-BOO-004": (
        "Vas a ver cómo una negación atraviesa una expresión: niega cada término y cambia AND por OR, o viceversa.",
        "Cambia A y B o recorre la transformación paso a paso; ambos lados siempre producen el mismo resultado.",
    ),
    "ALG-BOO-005": (
        "Vas a ver que XOR se activa solo cuando las entradas son distintas.",
        "Cambia A y B: si exactamente una entrada vale 1, la salida se enciende.",
    ),
    "ALG-BOO-006": (
        "Vas a ver que un término más específico no añade nada cuando ya está contenido en otro.",
        "Cambia A y B y observa cómo AB nunca agrega un caso que A no tenga ya.",
    ),
    "ALG-BOO-007": (
        "Vas a ver que en una suma de productos cada fila donde F=1 genera un minterm.",
        "Cambia la salida de la función y observa cómo cada uno se convierte en un producto AND; después todos se unen mediante OR.",
    ),
    "ALG-BOO-008": (
        "Vas a ver que en una forma producto de sumas cada fila donde F=0 genera un maxterm.",
        "Cambia la salida de la función y observa cómo cada cero se convierte en una cláusula OR; después todas las cláusulas se multiplican.",
    ),
    "ALG-BOO-009": (
        "Vas a ver que dos expresiones booleanas son equivalentes cuando producen exactamente las mismas salidas para todas las combinaciones posibles.",
...
COPY_I18N: dict[str, dict[str, tuple[str, str]]] = {
    "en": {
        ...
        "ALG-INE-002": (
            "The solution is a set of x values (intervals), not a “region of the plane”.",
            "Pick the inequality and move a,b,c: use the parabola for the sign and read the solution on the number line.",
        ),
        "ALG-INE-003": (
            "Denominator zeros never belong to the solution; the answer is a union of intervals.",
        ...
        "ALG-LOG-001": (
        "Each step of one unit in x multiplies the value by the base a: if a>1 it grows and if 0<a<1 it decays.",
        "Move base a and value x. Watch (0,1), the asymptote y=0, and how the curve grows or decays.",
    ),
        "ALG-LOG-002": (
        "The logarithm answers: “to what power do I raise the base to get x?”.",
        "Move the base and x. If log_b(x)=y, then b^y=x. Points (x,y) and (y,x) mirror across y=x.",
    ),
        "ALG-LOG-007": (
        "The sign of k decides whether the quantity grows, decays, or stays constant.",
        "Move k: positive grows, negative decays, k=0 stays flat. Move t and watch P(t)=P₀ e^{kt}.",
    ),
        "ALG-COM-001": (
        ...
        "ALG-BOO-001": (
            "You will see that 0 and 1 behave oppositely in OR and AND: one can leave A unchanged or dominate the result.",
            "Toggle A between 0 and 1 and notice which operations keep its value and which force the output.",
        ),
        "ALG-BOO-002": (
            "You will see two patterns: repeating A keeps its value; combining A with its complement always gives 1 with OR and 0 with AND.",
            "Toggle A and watch which results depend on A and which stay constant.",
        ),
        "ALG-BOO-003": (
            "You will see how one term distributes over each branch of a Boolean expression.",
            "Distribute A step by step and change A, B, and C to verify both forms always match.",
        ),
        "ALG-BOO-004": (
            "You will see how negation crosses a group: negate each term and swap AND for OR, or vice versa.",
            "Change A and B or step through the transformation; both sides always match.",
        ),
        "ALG-BOO-005": (
            "You will see that XOR turns on only when the inputs differ.",
            "Change A and B: if exactly one input is 1, the output turns on.",
        ),
        "ALG-BOO-006": (
            "You will see that a more specific term adds nothing when it is already contained in another.",
            "Change A and B and notice how AB never adds a case that A does not already cover.",
        ),
        "ALG-BOO-007": (
            "You will see that in a sum of products each row where F=1 generates a minterm.",
            "Change the function output and watch each 1 become an AND product; then all products join with OR.",
        ),
        "ALG-BOO-008": (
            "You will see that in a product of sums each row where F=0 generates a maxterm.",
            "Change the function output and watch each 0 become an OR clause; then all clauses multiply.",
        ),
        "ALG-BOO-009": (
            "You will see that two Boolean expressions are equivalent when they produce the same outputs for every assignment.",
            "Compare F and G row by row. One mismatch is enough to disprove equivalence.",
        ),
        "ALG-MOD-001": (
    ...
    "de": {
        ...
        "ALG-LOG-001": (
        "Jeder Schritt um eine Einheit in x multipliziert den Wert mit der Basis a: bei a>1 wächst sie, bei 0<a<1 fällt sie.",
        "Bewege Basis a und Wert x. Beobachte (0,1), die Asymptote y=0 und ob die Kurve wächst oder fällt.",
    ),
        "ALG-LOG-002": (
        "Der Logarithmus antwortet: „Mit welchem Exponenten erhebe ich die Basis, um x zu bekommen?“.",
        "Bewege Basis und x. Wenn log_b(x)=y, dann b^y=x. Punkte (x,y) und (y,x) spiegeln sich an y=x.",
    ),
        "ALG-LOG-007": (
        "Das Vorzeichen von k entscheidet, ob die Größe wächst, fällt oder konstant bleibt.",
        "Bewege k: positiv wächst, negativ fällt, k=0 bleibt flach. Bewege t und sieh P(t)=P₀ e^{kt}.",
    ),
        "ALG-COM-001": (
    ...
    "fr": {
        ...
        "ALG-LOG-001": (
        "Chaque pas d’une unité en x multiplie la valeur par la base a : si a>1 ça croît, si 0<a<1 ça décroît.",
        "Bouge la base a et la valeur x. Observe (0,1), l’asymptote y=0 et si la courbe croît ou décroît.",
    ),
        "ALG-LOG-002": (
        "Le logarithme répond : « à quel exposant j’élève la base pour obtenir x ? ».",
        "Bouge la base et x. Si log_b(x)=y, alors b^y=x. Les points (x,y) et (y,x) se reflètent par rapport à y=x.",
    ),
        "ALG-LOG-007": (
        "Le signe de k décide si la quantité croît, décroît ou reste constante.",
        "Bouge k : positif croît, négatif décroît, k=0 reste plat. Bouge t et observe P(t)=P₀ e^{kt}.",
    ),
        "ALG-COM-001": (
    ...
    "it": {
        ...
        "ALG-LOG-001": (
        "Ogni passo di un’unità in x moltiplica il valore per la base a: se a>1 cresce, se 0<a<1 decresce.",
        "Muovi la base a e il valore x. Osserva (0,1), l’asintoto y=0 e se la curva cresce o decresce.",
    ),
        "ALG-LOG-002": (
        "Il logaritmo risponde: «a quale esponente elevo la base per ottenere x?».",
        "Muovi la base e x. Se log_b(x)=y, allora b^y=x. I punti (x,y) e (y,x) si riflettono rispetto a y=x.",
    ),
        "ALG-LOG-007": (
        "Il segno di k decide se la quantità cresce, decresce o resta costante.",
        "Muovi k: positivo cresce, negativo decresce, k=0 resta piatta. Muovi t e osserva P(t)=P₀ e^{kt}.",
    ),
        "ALG-COM-001": (
    ...
    "pt": {
        ...
        "ALG-LOG-001": (
        "Cada avanço de uma unidade em x multiplica o valor pela base a: se a>1 cresce e se 0<a<1 decresce.",
        "Move a base a e o valor x. Observa (0,1), a assíntota y=0 e como a curva cresce ou decresce.",
    ),
        "ALG-LOG-002": (
        "O logaritmo responde: «a que expoente elevo a base para obter x?».",
        "Move a base e x. Se log_b(x)=y, então b^y=x. Os pontos (x,y) e (y,x) reflectem-se em y=x.",
    ),
        "ALG-LOG-007": (
        "O sinal de k decide se a quantidade cresce, decresce ou permanece constante.",
        "Move k: positivo cresce, negativo decresce e k=0 fica constante. Move t e observa P(t)=P₀ e^{kt}.",
    ),
        "ALG-COM-001": (
