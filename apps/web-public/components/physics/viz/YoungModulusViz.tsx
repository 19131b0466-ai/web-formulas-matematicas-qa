...
import { stress, youngModulus } from './physLote3Math';
...
export function YoungModulusViz({ mode }: { mode?: string }) {
  const tr = useTranslations('vizFisica.l3.equ005');
  const uid = useId();
  ...
  const Y = YMAT[mat]!;
  const sigma = stress(F, A);
  const eps = sigma / Y;
  const dL = eps * L0;
  const vis = 1 + Math.min(0.8, eps * (mat === 'goma' ? 80 : 4000));
  const Ycalc = youngModulus(sigma, eps);
  ...
  return (
    <VizPanel>
      <div className="space-y-4">
        ...
        <ButtonRow>
          <VizButton active={mat === 'acero'} onClick={() => setMat('acero')}>{tr('steel')}</VizButton>
          <VizButton active={mat === 'goma'} onClick={() => setMat('goma')}>{tr('rubber')}</VizButton>
        </ButtonRow>
        <svg viewBox="0 0 420 120" className="h-auto w-full" role="img" aria-label={tr('aria')}>
          ...
          <text x={200} y={100} fontSize={11} fill={MUTED}>{tr('caption')}</text>
        </svg>
        <PhysResult
          primary={mode === 'stress' ? tr('stressOnly', { sigma: present(sigma) }) : mode === 'strain' ? tr('strainOnly', { eps: present(eps) }) : tr('young', { Y: present(Ycalc) })}
          secondary={tr('readout', { sigma: present(sigma), eps: present(eps), dL: present(dL) })}
        />
        <PhysStatus id={uid}>
          {mode === 'stress'
            ? tr('stressStatus', { sigma: present(sigma) })
            : mode === 'strain'
              ? tr('strainStatus', { eps: present(eps), dL: present(dL) })
              : tr('status', { sigma: present(sigma), eps: present(eps), dL: present(dL) })}
        </PhysStatus>
        <ControlsStack>
