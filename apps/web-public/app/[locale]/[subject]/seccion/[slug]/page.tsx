...
export async function generateStaticParams() {
  const subjects: SubjectSlug[] = ['calculo-ii', 'fisica-basica'];
  const params: Array<{ locale: string; subject: string; slug: string }> = [];
  for (const subject of subjects) {
    const sections = await fetchSections(subject);
    const slugs = flattenSections(sections)
      .filter((s) => !isAppendixSlug(s.slug) && s.slug !== 'lista-comprobacion')
      .map((s) => s.slug);
    for (const locale of routing.locales) {
      for (const slug of slugs) {
        params.push({ locale, subject, slug });
      }
    }
  }
  return params;
